...
def log_in():
    ...
    try:
        ...
        page.wait_for_url(url="http://newonpan.getmall.kr/main/main.php", timeout=20000)    # working", 5000 --> 20000
...
def get_product_stock(keyword):
    ...
    page.wait_for_url(search_url, timeout=20000)    # ?? --> 20000
    ...
    while True:
        ...
        if browser.page().locator(f'{href_link}').is_visible(timeout=1000):      # 500 --> 1000
            ...
            page.wait_for_url(url=pg_url, timeout=10000)    # working, 5000 --> 10000
            continue
        
        # next, click "next" 
        # 'prList' 클래스를 가진 <font> 태그 안에 텍스트 '[next]'를 포함하는 요소를 찾아서 클릭
        if browser.page().locator("font.prList:has-text('[next]')").is_visible(timeout=1000):        # 500 --> 1000
        ...
        # 더 간단하게 텍스트만으로 찾기
        if browser.page().locator("[next]").is_visible(timeout=1000):        # 500 --> 1000
...
def extract_table_data(page_no, keyword):
    ...
    for row_index, row_element in enumerate(tr_rows):
        ...
        if row_element.locator("p.prname").is_visible(timeout=1000):     # 500 --> 1000   
            prod_name = row_element.locator("p.prname").text_content(timeout=1000)      # 500 --> 1000
        ...
        for cell_index, cell_element in enumerate(td_cells):
            ...
            # if cell_element.locator('a[href]').is_visible(timeout=1000):   # 500 --> 1000
            #     href_link = cell_element.locator('a[href]').get_attribute('href', timeout=1000)    # 500 --> 1000
            ...
            xpath_str = "a[href]"   
            if cell_element.locator(xpath_str).is_visible(timeout=1000): # 500 --> 1000
                temp_url = cell_element.locator(xpath_str).get_attribute('href', timeout=1000)   # 500 --> 1000
            ...
            xpath_str = 'p.prprice'
            if cell_element.locator(xpath_str).is_visible(timeout=1000):         # 500 --> 1000
                prprice = cell_element.locator(xpath_str).text_content(timeout=1000)     # 500 --> 1000
...
def get_prod_option_data(page_no, href_url, keyword):
    ...
    page.wait_for_url(url=absolute_url, timeout=100000)          # 5000 --> 100000 
    
    # access invenrory and save them to excel file
    save_data_to_excel(keyword)
    
    # print("will go back to prod page")
    browser.page().go_back(timeout=10000)                # 1000 --> 10000
...
def save_data_to_excel(keyword):
    ...
    prname = page.locator("div.prdetailname").text_content(timeout=1000)        # 500 --> 1000
    prprice = page.locator("span#idx_price").inner_text(timeout=1000)            # 500 --> 1000
